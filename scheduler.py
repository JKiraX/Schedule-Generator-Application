...
class AdaptiveScheduler:
    ...
    def create_two_month_shift_schedule(self, employees, shifts, start_date):
        ...
        for month in range(num_months):
            ...
            # Create working day variables for each employee and day
            # This indicates if the employee is working any shift on that day
            working_day = {}
            for e in range(num_employees):
                for d in range(num_days_per_month):
                    working_day[(e, d)] = csp_model.NewBoolVar(f'working_e{e}_d{d}')
                    # Link with shift variables
                    csp_model.Add(sum(shift_vars[(e, d, s)] for s in range(num_shifts)) == working_day[(e, d)])
            ...
            for e in range(num_employees):
                ...
                if emp_id in employee_preferences and 'consecutive' in employee_preferences[emp_id]:
                    ...
                    consecutive_prefs[e] = {'min': 3, 'max': 3, 'preferred': 3}  # Default to 3 consecutive days
            
            # Constraints: Implement the 3-days-on, 1-day-off pattern
            for e in range(num_employees):
                for d in range(num_days_per_month - 4):  # Check each 4-day window
                    # If employee works 3 days straight, they must have the 4th day off
                    # working_day[(e, d)] and working_day[(e, d+1)] and working_day[(e, d+2)] => not working_day[(e, d+3)]
                    csp_model.AddBoolOr([
                        csp_model.NewBoolVar('not_working_' + str(e) + '_' + str(d)).Not(),
                        csp_model.NewBoolVar('not_working_' + str(e) + '_' + str(d+1)).Not(),
                        csp_model.NewBoolVar('not_working_' + str(e) + '_' + str(d+2)).Not(),
                        csp_model.NewBoolVar('not_working_' + str(e) + '_' + str(d+3))
                    ])

            # Constraints: At least one person per shift - CRITICAL FIX
            for d in range(num_days_per_month):
                for s in range(num_shifts):
                    # Each shift must have at least one employee every day - no exceptions
            ...
            morning_shift = next((i for i, s in enumerate(shifts) if s['start_time'].strftime('%H:%M') == '06:00'), None)
            day_shift = next((i for i, s in enumerate(shifts) if s['start_time'].strftime('%H:%M') == '08:00'), None)

            # Define incompatible shift sequences
            incompatible_sequences = []
            
            # Night shift (10pm-6am) followed by morning (6am-2pm) or day shift (8am-4pm)
            if night_shift is not None:
                if morning_shift is not None:
                    incompatible_sequences.append((night_shift, morning_shift))
                if day_shift is not None:
                    incompatible_sequences.append((night_shift, day_shift))
            
            # Evening shift (2pm-10pm) followed by morning shift (6am-2pm)
            if evening_shift is not None and morning_shift is not None:
                incompatible_sequences.append((evening_shift, morning_shift))
            
            # Apply the constraints for incompatible sequences
            for e in range(num_employees):
                for d in range(num_days_per_month - 1):
                    for s1, s2 in incompatible_sequences:
                        # If employee works shift s1 on day d, they cannot work shift s2 on day d+1
                        csp_model.Add(shift_vars[(e, d, s1)] + shift_vars[(e, d+1, s2)] <= 1)

            # Constraints: Ensure at least 2 days off per week (5 work days per week)
            for e in range(num_employees):
                for w in range(4):  # 4 weeks
                    week_start = w * 7
                    week_end = min(week_start + 7, num_days_per_month)
                    # Each employee works exactly 5 days per week
                    if week_end - week_start == 7:  # Only enforce for full weeks
                        csp_model.Add(sum(working_day[(e, d)] for d in range(week_start, week_end)) == 5)
                    else:  # For partial weeks, prorate
                        days_in_week = week_end - week_start
                        work_days = max(1, min(days_in_week - 1, int(5 * days_in_week / 7)))
                        csp_model.Add(sum(working_day[(e, d)] for d in range(week_start, week_end)) == work_days)

            # Solve the CSP model with a time limit
            csp_solver = cp_model.CpSolver()
            csp_solver.parameters.max_time_in_seconds = 90  # 90 second time limit
            ...
            # At least one person per shift - CRITICAL FIX
            for d in range(num_days_per_month):
                for s in range(num_shifts):
                    ilp_solver.Add(sum(ilp_vars[(e, d, s)] for e in range(num_employees)) >= 1)

            # Incompatible shift sequences 
            for e in range(num_employees):
                for d in range(num_days_per_month - 1):
                    for s1, s2 in incompatible_sequences:
                        ilp_solver.Add(ilp_vars[(e, d, s1)] + ilp_vars[(e, d+1, s2)] <= 1)

            # Weekly work pattern constraints
            for e in range(num_employees):
                for w in range(4):  # 4 weeks
                    week_start = w * 7
                    week_end = min(week_start + 7, num_days_per_month)
                    if week_end - week_start == 7:  # Only enforce for full weeks
                        ...
                        days_in_week = week_end - week_start
                        work_days = max(1, min(days_in_week - 1, int(5 * days_in_week / 7)))
                        ilp_solver.Add(sum(ilp_vars[(e, d, s)] for d in range(week_start, week_end) for s in range(num_shifts)) == work_days)
        ...
        # Validate the schedule to ensure no shifts are unfilled
        self._validate_shift_coverage(full_schedule, employees, shifts, start_date, num_days_per_month * num_months)
        
        return full_schedule

    def _validate_shift_coverage(self, schedule, employees, shifts, start_date, num_days):
        """
        Validate that every shift has at least one employee assigned.
        If any shifts are unfilled, assign employees to them.
        """
        # Initialize coverage tracking
        coverage = {}
        for d in range(num_days):
            coverage[d] = {s['id']: 0 for s in shifts}
        
        # Count employees assigned to each shift
        for emp_id, emp_schedule in schedule.items():
            for day, shift_id in emp_schedule.items():
                if day < num_days:  # Only consider days within our planning horizon
                    coverage[day][shift_id] += 1
        
        # Check for unfilled shifts
        unfilled_shifts = []
        for d in range(num_days):
            for s in shifts:
                if coverage[d][s['id']] == 0:
                    unfilled_shifts.append((d, s['id']))
        
        # If there are unfilled shifts, assign employees to them
        if unfilled_shifts:
            logger.warning(f"Found {len(unfilled_shifts)} unfilled shifts. Adding emergency assignments.")
            
            # Simple round-robin assignment for unfilled shifts
            emp_index = 0
            for day, shift_id in unfilled_shifts:
                # Find an employee who isn't already working that day
                assigned = False
                for _ in range(len(employees)):
                    emp_id = employees[emp_index]['id']
                    emp_index = (emp_index + 1) % len(employees)
                    
                    # Check if employee is already working that day
                    if emp_id in schedule and day in schedule[emp_id]:
                        continue
                    
                    # Assign the shift
                    if emp_id not in schedule:
                        schedule[emp_id] = {}
                    schedule[emp_id][day] = shift_id
                    assigned = True
                    break
                
                # If no employee can be assigned without conflicts, just assign someone
                if not assigned:
                    emp_id = employees[emp_index]['id']
                    emp_index = (emp_index + 1) % len(employees)
                    ...
                    if emp_id not in schedule:
                        schedule[emp_id] = {}
                    schedule[emp_id][day] = shift_id
                    logger.warning(f"Had to assign employee {emp_id} to an additional shift on day {day}")
    ...
    def _create_schedule_with_relaxed_constraints(self, employees, shifts, start_date, employee_preferences, employee_availability):
        ...
        for month in range(num_months):
            ...
            # At least one person per shift (essential) - CRITICAL FIX
            for d in range(num_days_per_month):
                for s in range(num_shifts):
                    csp_model.Add(sum(shift_vars[(e, d, s)] for e in range(num_employees)) >= 1)
            ...
            # Night shift and Evening shift restrictions (important for health)
            night_shift = next((i for i, s in enumerate(shifts) if s['start_time'].strftime('%H:%M') == '22:00'), None)
            evening_shift = next((i for i, s in enumerate(shifts) if s['start_time'].strftime('%H:%M') == '14:00'), None)
            morning_shift = next((i for i, s in enumerate(shifts) if s['start_time'].strftime('%H:%M') == '06:00'), None)
            day_shift = next((i for i, s in enumerate(shifts) if s['start_time'].strftime('%H:%M') == '08:00'), None)
            ...
            # Try to balance employee workload (not critical but desirable)
            for e in range(num_employees):
                # Each employee should work between 15-20 days per month
                csp_model.Add(sum(shift_vars[(e, d, s)] for d in range(num_days_per_month) for s in range(num_shifts)) >= 15)
                csp_model.Add(sum(shift_vars[(e, d, s)] for d in range(num_days_per_month) for s in range(num_shifts)) <= 22)
            
            # Solve the relaxed CSP model
            csp_solver = cp_model.CpSolver()
            csp_solver.parameters.max_time_in_seconds = 60  # Set a time limit
            csp_status = csp_solver.Solve(csp_model)
            ...
            for d in range(num_days_per_month):
                for s in range(num_shifts):
                    ...
                    if not assigned:
                        ...
                        # If still not assigned, assign to any employee as a last resort
                        if not assigned:
                            emp_id = employees[0]['id']  # Just assign to the first employee
                            day_number = d + month * num_days_per_month
                            full_schedule[emp_id][day_number] = shifts[s]['id']
                            logger.warning(f"Had to assign employee {emp_id} to an additional shift on day {day_number}")
